...
dui_folder="${DUI_MAIN_FOLDER:=/usr/local/d-ui}"
dui_service="${DUI_SERVICE:=/etc/systemd/system}"
...
# Non-interactive mode: triggered explicitly via DUI_NONINTERACTIVE=1, or
...
if [[ "${DUI_NONINTERACTIVE:-0}" == "1" ]] || [[ ! -t 0 ]]; then
    NONINTERACTIVE=1
...
install_base() {
    case "${release}" in
        ubuntu | debian | armbian)
            # Use force-confdef to avoid interactive prompts in CI
            apt-get update && apt-get install -y -q -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" cron curl tar tzdata socat ca-certificates openssl
            ;;
...
# Postgres env file (/etc/default/d-ui).
write_install_result() {
    local u="$1" p="$2" port="$3" wbp="$4" scheme="$5" host="$6" token="$7" dbtype="$8"
    local result_file="/etc/d-ui/install-result.env"
    local url_host="${host:-SERVER_IP_UNKNOWN}"
    install -d -m 755 /etc/d-ui 2> /dev/null
    local prev_umask
    prev_umask=$(umask)
    umask 077
    if ! {
        printf 'DUI_USERNAME=%q\n' "$u"
        printf 'DUI_PASSWORD=%q\n' "$p"
        printf 'DUI_PANEL_PORT=%q\n' "$port"
        printf 'DUI_WEB_BASE_PATH=%q\n' "$wbp"
        printf 'DUI_ACCESS_URL=%q\n' "${scheme}://${url_host}:${port}/${wbp}"
        printf 'DUI_API_TOKEN=%q\n' "$token"
        printf 'DUI_DB_TYPE=%q\n' "$dbtype"
    } > "$result_file"; then
        umask "$prev_umask"
...
install_postgres_local() {
    ...
    case "${release}" in
        ubuntu | debian | armbian)
            apt-get update >&2 && apt-get install -y -q -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" postgresql >&2 || return 1
            ;;
        fedora | amzn | virtuozzo | rhel | almalinux | rocky | ol)
...
ensure_pg_client() {
    ...
    case "${release}" in
        ubuntu | debian | armbian)
            apt-get update >&2 && apt-get install -y -q -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" postgresql-client >&2 || return 1
            ;;
        fedora | amzn | virtuozzo | rhel | almalinux | rocky | ol)
...
setup_ssl_certificate() {
    ...
    if [ $? -ne 0 ]; then
        ...
        echo -e "${yellow}Please ensure port 80 is open and try again later with: d-ui${plain}"
    ...
    ~/.acme.sh/acme.sh --installcert --force -d ${domain} \
        ...
        --reloadcmd "systemctl restart d-ui" > /dev/null 2>&1
    ...
    if [[ -f "$webCertFile" && -f "$webKeyFile" ]]; then
        ${dui_folder}/d-ui cert -webCert "$webCertFile" -webCertKey "$webKeyFile" > /dev/null 2>&1
...
setup_ip_certificate() {
    ...
    local reloadCmd="systemctl restart d-ui 2>/dev/null || rc-service d-ui restart 2>/dev/null || true"

    # Choose port for HTTP-01 listener (default 80, prompt override)
    local WebPort=""
    prompt_or_default WebPort "Port to use for ACME HTTP-01 listener (default 80): " "80" DUI_ACME_HTTP_PORT
    WebPort="${WebPort:-80}"
    ...
    ~/.acme.sh/acme.sh --set-default-ca --server letsencrypt --force > /dev/null 2>&1
    [[ -n "${DUI_ACME_EMAIL:-}" ]] && ~/.acme.sh/acme.sh --register-account -m "${DUI_ACME_EMAIL}" > /dev/null 2>&1
    ...
    ${dui_folder}/d-ui cert -webCert "${certDir}/fullchain.pem" -webCertKey "${certDir}/privkey.pem"
    ...
    echo -e "${yellow}acme.sh will automatically renew and reload d-ui before expiry.${plain}"
    return 0
}

# Comprehensive manual SSL certificate issuance via acme.sh
ssl_cert_issue() {
    local existing_webBasePath=$(${dui_folder}/d-ui setting -show true | grep 'webBasePath:' | awk -F': ' '{print $2}' | tr -d '[:space:]' | sed 's#^/##')
    local existing_port=$(${dui_folder}/d-ui setting -show true | grep 'port:' | awk -F': ' '{print $2}' | tr -d '[:space:]')
    ...
    if [[ "$NONINTERACTIVE" == "1" ]]; then
        ...
        if [[ -z "$domain" ]] || ! is_domain "$domain"; then
            echo -e "${red}DUI_SSL_MODE=domain requires a valid DUI_DOMAIN (got: '${DUI_DOMAIN:-}').${plain}"
            return 1
        fi
    else
        while true; do
    ...
    local WebPort=80
    prompt_or_default WebPort "Please choose which port to use (default is 80): " "80" DUI_ACME_HTTP_PORT
    ...
    systemctl stop d-ui 2> /dev/null || rc-service d-ui stop 2> /dev/null
    ...
    if [[ ${cert_exists} -eq 0 ]]; then
        ...
        if [ $? -ne 0 ]; then
            ...
            rm -rf ~/.acme.sh/${domain} ~/.acme.sh/${domain}_ecc
            systemctl start d-ui 2> /dev/null || rc-service d-ui start 2> /dev/null
            return 1
        else
    ...
    reloadCmd="systemctl restart d-ui || rc-service d-ui restart"
    echo -e "${green}Default --reloadcmd for ACME is: ${yellow}systemctl restart d-ui || rc-service d-ui restart${plain}"
    ...
    if [[ "$setReloadcmd" == "y" || "$setReloadcmd" == "Y" ]]; then
        echo -e "\n${green}\t1.${plain} Preset: systemctl reload nginx ; systemctl restart d-ui"
        echo -e "${green}\t2.${plain} Input your own command"
        echo -e "${green}\t0.${plain} Keep default reloadcmd"
        read -rp "Choose an option: " choice
        case "$choice" in
            1)
                echo -e "${green}Reloadcmd is: systemctl reload nginx ; systemctl restart d-ui${plain}"
                reloadCmd="systemctl reload nginx ; systemctl restart d-ui"
                ;;
            2)
                echo -e "${yellow}It's recommended to put d-ui restart at the end${plain}"
    ...
    if [[ "$setPanel" == "y" || "$setPanel" == "Y" ]]; then
        ...
        if [[ -f "$webCertFile" && -f "$webKeyFile" ]]; then
            ${dui_folder}/d-ui cert -webCert "$webCertFile" -webCertKey "$webKeyFile"
...
prompt_and_setup_ssl() {
    ...
    if [[ "$NONINTERACTIVE" == "1" ]]; then
        case "${DUI_SSL_MODE:-none}" in
            domain) ssl_choice="1" ;;
            ip) ssl_choice="2" ;;
            none | "") ssl_choice="4" ;;
            *)
                echo -e "${yellow}Unknown DUI_SSL_MODE='${DUI_SSL_MODE}', defaulting to none (HTTP).${plain}"
                ssl_choice="4"
                ;;
        esac
    else
    ...
    case "$ssl_choice" in
        ...
        2)
            ...
            local ipv6_addr=""
            prompt_or_default ipv6_addr "Do you have an IPv6 address to include? (leave empty to skip): " "" DUI_SSL_IPV6
            ipv6_addr="${ipv6_addr// /}" # Trim whitespace

            # Stop panel if running (port 80 needed)
            if [[ $release == "alpine" ]]; then
                rc-service d-ui stop > /dev/null 2>&1
            else
                systemctl stop d-ui > /dev/null 2>&1
            fi
        ...
        3)
            ...
            # 3.4 Apply Settings via d-ui binary
            ${dui_folder}/d-ui cert -webCert "$custom_cert" -webCertKey "$custom_key" > /dev/null 2>&1
            ...
            systemctl restart d-ui > /dev/null 2>&1 || rc-service d-ui restart > /dev/null 2>&1
            ;;
        4)
            echo ""
            ...
            if [[ "$bind_local" == "y" || "$bind_local" == "Y" ]]; then
                ${dui_folder}/d-ui setting -listenIP "127.0.0.1" > /dev/null 2>&1
                SSL_HOST="127.0.0.1"
...
config_after_install() {
    local existing_hasDefaultCredential=$(${dui_folder}/d-ui setting -show true | grep -Eo 'hasDefaultCredential: .+' | awk '{print $2}')
    ...
    local existing_cert=$(${dui_folder}/d-ui setting -getCert true | grep 'cert:' | awk -F': ' '{print $2}' | tr -d '[:space:]')
    local URL_lists=(
    ...
    if [[ -z "$server_ip" ]]; then
        if [[ "$NONINTERACTIVE" == "1" ]]; then
            ...
            # displayed access URL. Fall back to DUI_SERVER_IP or leave blank.
            server_ip="${DUI_SERVER_IP:-}"
        else
    ...
    if [[ ${#existing_webBasePath} -lt 4 ]]; then
        if [[ "$existing_hasDefaultCredential" == "true" ]]; then
            local config_webBasePath="${DUI_WEB_BASE_PATH:-$(gen_random_string 18)}"
            local config_username="${DUI_USERNAME:-$(gen_random_string 10)}"
            local config_password="${DUI_PASSWORD:-$(gen_random_string 10)}"
            local config_port=""

            local db_label="SQLite (/etc/d-ui/d-ui.db)"
            echo ""
            echo -e "${green}═══════════════════════════════════════════${plain}"
            ...
            if [[ "$NONINTERACTIVE" == "1" ]]; then
                if [[ "${DUI_DB_TYPE:-sqlite}" == "postgres" ]]; then
                    db_choice="2"
                else
                    db_choice="1"
                fi
            ...
            if [[ "$db_choice" == "2" ]]; then
                ...
                case "${release}" in
                    ubuntu | debian | armbian)
                        dui_env_file="/etc/default/d-ui"
                        ;;
                    arch | manjaro | parch | alpine)
                        dui_env_file="/etc/conf.d/d-ui"
                        ;;
                    *)
                        dui_env_file="/etc/sysconfig/d-ui"
                        ;;
                esac
                ...
                while [[ -z "$dui_dsn" ]]; do
                    if [[ "$NONINTERACTIVE" == "1" ]]; then
                        ...
                        local pg_cred_file
                        pg_cred_file=$(mktemp 2> /dev/null) || pg_cred_file=$(mktemp -t d-ui-pg-creds.XXXXXXXX)
                        if [[ -n "${pg_cred_file}" ]] && dui_dsn=$(PG_CRED_FILE="${pg_cred_file}" install_postgres_local); then
                        ...
                        echo -e "${yellow}Set DUI_DB_DSN to use an existing server, or DUI_DB_TYPE=sqlite.${plain}"
                        exit 1
                    fi
                    echo ""
                    ...
                    if [[ "$pg_mode" == "2" ]]; then
                        while [[ -z "$dui_dsn" ]]; do
                            read -rp "Enter PostgreSQL DSN (postgres://user:pass@host:port/dbname?sslmode=disable): " dui_dsn
                            dui_dsn="${dui_dsn// /}"
                        done
                        db_label="PostgreSQL (external)"
                    else
                        ...
                        if dui_dsn=$(PG_CRED_FILE="${pg_cred_file}" install_postgres_local); then
                            pg_local_installed=1
                            if [[ -r "${pg_cred_file}" ]]; then
                                # shellcheck disable=SC1090
                                source "${pg_cred_file}"
                ...
                if [[ -n "$dui_dsn" ]]; then
                    install -d -m 755 "$(dirname "$dui_env_file")"
                    umask 077
                    cat > "$dui_env_file" << EOF
DUI_DB_TYPE=postgres
DUI_DB_DSN=${dui_dsn}
EOF
                    chmod 600 "$dui_env_file"
                    umask 022
                    export DUI_DB_TYPE=postgres
                    export DUI_DB_DSN="${dui_dsn}"
            ...
            if [[ "$NONINTERACTIVE" == "1" ]]; then
                if [[ -n "${DUI_PANEL_PORT:-}" ]]; then
                    config_port="${DUI_PANEL_PORT}"
            ...
            ${dui_folder}/d-ui setting -username "${config_username}" -password "${config_password}" -port "${config_port}" -webBasePath "${config_webBasePath}"
            ...
            local config_apiToken=$(${dui_folder}/d-ui setting -getApiToken true | grep -Eo 'apiToken: .+' | awk '{print $2}')
            ...
            if [[ "$db_choice" == "2" && "$pg_local_installed" == "1" ]]; then
                ...
                echo -e "${green}DSN:        ${dui_dsn}${plain}"
                echo -e "${green}Env file:   ${dui_env_file}${plain}"
                echo -e "${green}-------------------------------------------${plain}"
                ...
                echo -e "${green}═══════════════════════════════════════════${plain}"
                echo -e "${yellow}⚠ The panel reads these credentials from ${dui_env_file}.${plain}"
            ...
            ${dui_folder}/d-ui setting -webBasePath "${config_webBasePath}"
        ...
        if [[ "$existing_hasDefaultCredential" == "true" ]]; then
            ...
            ${dui_folder}/d-ui setting -username "${config_username}" -password "${config_password}"
            ...
            config_apiToken=$(${dui_folder}/d-ui setting -getApiToken true | grep -Eo 'apiToken: .+' | awk '{print $2}')
            : "${SSL_SCHEME:=https}"
            : "${SSL_HOST:=${server_ip}}"
            write_install_result "${config_username}" "${config_password}" "${existing_port}" \
                "${existing_webBasePath}" "${SSL_SCHEME}" "${SSL_HOST}" "${config_apiToken}" "${DUI_DB_TYPE:-sqlite}"
        else
        ...
        # Properly detect empty cert by checking if cert: line exists and has content after it
        existing_cert=$(${dui_folder}/d-ui setting -getCert true | grep 'cert:' | awk -F': ' '{print $2}' | tr -d '[:space:]')
        if [[ -z "$existing_cert" ]]; then
            echo ""
            echo -e "${green}═══════════════════════════════════════════${plain}"
            echo -e "${green}     SSL Certificate Setup (RECOMMENDED)   ${plain}"
...
# by invoking the freshly installed d-ui CLI. IP Limit is load-bearing on
...
setup_fail2ban() {
    if [[ -n "${DUI_ENABLE_FAIL2BAN+x}" && "${DUI_ENABLE_FAIL2BAN}" != "true" ]]; then
        echo -e "${yellow}DUI_ENABLE_FAIL2BAN=${DUI_ENABLE_FAIL2BAN}, skipping Fail2ban auto-setup.${plain}"
        return 0
    fi

    if [[ ! -x /usr/bin/d-ui ]]; then
        echo -e "${yellow}d-ui CLI not found; skipping Fail2ban auto-setup.${plain}"
        return 0
    fi

    echo -e "${green}Setting up Fail2ban for the IP Limit feature...${plain}"
    if /usr/bin/d-ui setup-fail2ban; then
        echo -e "${green}Fail2ban setup complete.${plain}"
    else
        echo -e "${yellow}Fail2ban setup did not finish; IP Limit stays disabled until you run 'd-ui' and open the IP Limit menu. Continuing.${plain}"
    fi
    return 0
}

# Lands a systemd unit file at ${dui_service}/d-ui.service via a temp file +
...
# /usr/bin/d-ui elsewhere in this script. source_is_url picks cp (from a
# file already extracted from the release tarball) vs curl (GitHub fallback).
_install_dui_service_unit() {
    local source="$1"
    local source_is_url="$2"
    local dest="${dui_service}/d-ui.service"
...
install_d-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        if [[ ! -s ${dui_folder}-linux-$(arch).tar.gz ]]; then
            rm ${dui_folder}-linux-$(arch).tar.gz -f
            echo -e "${red}Downloaded d-ui release archive is empty${plain}"
            exit 1
        fi
    else
        tag_version=$1
        ...
        if [[ $? -ne 0 ]]; then
            echo -e "${red}Download d-ui ${tag_version} failed, please check if the version exists ${plain}"
            exit 1
        fi
        if [[ ! -s ${dui_folder}-linux-$(arch).tar.gz ]]; then
            rm ${dui_folder}-linux-$(arch).tar.gz -f
    ...
    if [[ ! -s "${dui_script_temp}" ]]; then
        rm -f "${dui_script_temp}"
        echo -e "${red}Downloaded d-ui.sh is empty${plain}"
        exit 1
    fi

    # Stop d-ui service and remove old resources
    if [[ -e ${dui_folder}/ ]]; then
        if [[ $release == "alpine" ]]; then
            rc-service d-ui stop
        else
            systemctl stop d-ui
        fi
        # Kill any leftover mtg (MTProto) sidecars. d-ui runs them outside its own
    ...
    tar zxvf d-ui-linux-$(arch).tar.gz
    if [[ $? -ne 0 ]]; then
        rm d-ui-linux-$(arch).tar.gz -f
        rm -f "${dui_script_temp}"
        echo -e "${red}Failed to extract the d-ui release archive -- the previous installation has already been removed, so the panel will not start until this is fixed; try running the installer again${plain}"
        exit 1
    fi
    rm d-ui-linux-$(arch).tar.gz -f
    ...
    if [[ $? -ne 0 || ! -s d-ui ]]; then
        rm -f "${dui_script_temp}"
        echo -e "${red}Extracted d-ui archive is missing the d-ui binary -- the previous installation has already been removed, so the panel will not start until this is fixed; try running the installer again${plain}"
        exit 1
    fi
    chmod +x d-ui
    chmod +x d-ui.sh
    ...
    chmod +x d-ui bin/xray-linux-$(arch)
    ...
    # Update d-ui cli and se set permission
    mv -f "${dui_script_temp}" /usr/bin/d-ui
    if [[ $? -ne 0 ]]; then
        rm -f "${dui_script_temp}"
        echo -e "${red}Failed to install d-ui.sh${plain}"
        exit 1
    fi
    chmod +x /usr/bin/d-ui
    mkdir -p /var/log/d-ui
    config_after_install

    # Etckeeper compatibility
    if [ -d "/etc/.git" ]; then
        if [ -f "/etc/.gitignore" ]; then
            if ! grep -q "d-ui/d-ui.db" "/etc/.gitignore"; then
                echo "" >> "/etc/.gitignore"
                echo "d-ui/d-ui.db" >> "/etc/.gitignore"
                echo -e "${green}Added d-ui.db to /etc/.gitignore for etckeeper${plain}"
            fi
        else
            echo "d-ui/d-ui.db" > "/etc/.gitignore"
            echo -e "${green}Created /etc/.gitignore and added d-ui.db for etckeeper${plain}"
        fi
    fi

    if [[ $release == "alpine" ]]; then
        dui_rc_temp="/etc/init.d/d-ui.tmp.$$"
        rm -f "${dui_rc_temp}"
        if [[ -f "d-ui.rc" ]]; then
            echo -e "${green}Found d-ui.rc in extracted files, installing...${plain}"
        ...
        if [[ ! -s "${dui_rc_temp}" ]]; then
            rm -f "${dui_rc_temp}"
            echo -e "${red}Failed to obtain d-ui.rc${plain}"
            exit 1
        fi
        mv -f "${dui_rc_temp}" /etc/init.d/d-ui
        if [[ $? -ne 0 ]]; then
            rm -f "${dui_rc_temp}"
            echo -e "${red}Failed to install d-ui.rc${plain}"
            exit 1
        fi
        chmod +x /etc/init.d/d-ui
        rc-update add d-ui
        rc-service d-ui start
    else
        ...
        case "${release}" in
            ubuntu | debian | armbian)
                if [ -f "d-ui.service.debian" ]; then
                    echo -e "${green}Found d-ui.service.debian in extracted files, installing...${plain}"
                    if _install_dui_service_unit "d-ui.service.debian" "false"; then
                        service_installed=true
                    fi
                fi
                ;;
            arch | manjaro | parch)
                if [ -f "d-ui.service.arch" ]; then
                    echo -e "${green}Found d-ui.service.arch in extracted files, installing...${plain}"
                    if _install_dui_service_unit "d-ui.service.arch" "false"; then
                        service_installed=true
                    fi
                fi
                ;;
            *)
                if [ -f "d-ui.service.rhel" ]; then
                    echo -e "${green}Found d-ui.service.rhel in extracted files, installing...${plain}"
                    if _install_dui_service_unit "d-ui.service.rhel" "false"; then
                        service_installed=true
                    fi
                fi
                ;;
        ...
        if [ "$service_installed" = false ]; then
            if [ -f "d-ui.service" ]; then
                echo -e "${green}Found d-ui.service in extracted files, installing...${plain}"
                if _install_dui_service_unit "d-ui.service" "false"; then
                    service_installed=true
                fi
            fi
        fi

        # If service file still not found, download from GitHub
        if [ "$service_installed" = false ]; then
            ...
            if ! _install_dui_service_unit "$service_unit_url" "true"; then
                echo -e "${red}Failed to install d-ui.service from GitHub${plain}"
                exit 1
            fi
            service_installed=true
        fi
        ...
        if [ "$service_installed" = true ]; then
            ...
            chown root:root ${dui_service}/d-ui.service > /dev/null 2>&1
            chmod 644 ${dui_service}/d-ui.service > /dev/null 2>&1
            systemctl daemon-reload
            systemctl enable d-ui
            systemctl start d-ui
        else
            echo -e "${red}Failed to install d-ui.service file${plain}"
            exit 1
        fi
    fi

    # IP Limit relies on fail2ban; install + configure it now so the feature
    # works out of the box (no-op when DUI_ENABLE_FAIL2BAN=false). Never fatal.
    setup_fail2ban

    echo -e "${green}d-ui ${tag_version}${plain} installation finished, it is running now..."
    echo -e ""
    echo -e "┌───────────────────────────────────────────────────────┐
│  ${blue}d-ui control menu usages (subcommands):${plain}              │
│                                                       │
│  ${blue}d-ui${plain}              - Admin Management Script          │
│  ${blue}d-ui start${plain}        - Start                            │
│  ${blue}d-ui stop${plain}         - Stop                             │
│  ${blue}d-ui restart${plain}      - Restart                          │
│  ${blue}d-ui status${plain}       - Current Status                   │
│  ${blue}d-ui settings${plain}     - Current Settings                 │
│  ${blue}d-ui enable${plain}       - Enable Autostart on OS Startup   │
│  ${blue}d-ui disable${plain}      - Disable Autostart on OS Startup  │
│  ${blue}d-ui log${plain}          - Check logs                       │
│  ${blue}d-ui banlog${plain}       - Check Fail2ban ban logs          │
│  ${blue}d-ui update${plain}       - Update                           │
