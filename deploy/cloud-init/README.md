...
A single [`cloud-init.yaml`](cloud-init.yaml) user-data file that installs d-ui
...
2. cloud-init writes and runs `/opt/dui-bootstrap.sh`, which exports
   `DUI_NONINTERACTIVE=1` and pipes the project's `install.sh` into `bash`.
3. `install.sh` runs end-to-end with **zero prompts**, picking secure random
   values for any credential you didn't pin.
4. The generated credentials are written to `/etc/d-ui/install-result.env`
   (mode 600), echoed to the **serial console**, and appended to `/etc/motd`.

Retrieve them after boot with either:

```bash
sudo cat /etc/d-ui/install-result.env     # over SSH
```
...
Edit the `export DUI_*` lines inside the `write_files` block of
[`cloud-init.yaml`](cloud-init.yaml). All knobs are optional; unset ⇒ random/secure default.

| Env var | Default | Meaning |
| --- | --- | --- |
| `DUI_SSL_MODE` | `none` | `none` (plain HTTP), `ip` (Let's Encrypt IP cert), `domain` |
| `DUI_USERNAME` | random | Admin username |
| `DUI_PASSWORD` | random | Admin password |
| `DUI_PANEL_PORT` | random high port | Panel listen port |
| `DUI_WEB_BASE_PATH` | random | Panel base path (obscures the URL) |
| `DUI_DOMAIN` | — | Required when `DUI_SSL_MODE=domain` |
| `DUI_ACME_EMAIL` | — | Let's Encrypt account email (domain mode) |
| `DUI_DB_TYPE` / `DUI_DB_DSN` | `sqlite` | Set `postgres` + DSN to use PostgreSQL |
...
- **Google Cloud (GCE)** — `gcloud compute instances create dui \
