...
# DUI_NONINTERACTIVE=1, then asserts:
#   * /etc/d-ui/install-result.env exists (mode 600) with random, non-default creds
...
docker run --rm \
    ...
    -e DUI_NONINTERACTIVE=1 \
    -e DUI_SSL_MODE=none \
    -e DEBIAN_FRONTEND=noninteractive \
    "$IMAGE" bash -euo pipefail -c '
        apt-get update -qq
        # Fixes hanging debconf prompts during noninteractive installs
        apt-get install -y -qq -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" curl tar openssl ca-certificates > /dev/null
        ...
        echo "--- assertions ---"
        RESULT=/etc/d-ui/install-result.env
        ...
        . "$RESULT"
        [ -n "${DUI_USERNAME:-}" ] && [ "$DUI_USERNAME" != "admin" ] \
            || { echo "FAIL: username missing or still admin"; exit 1; }
        [ -n "${DUI_PASSWORD:-}" ] && [ "$DUI_PASSWORD" != "admin" ] \
            || { echo "FAIL: password missing or still admin"; exit 1; }
        [ -n "${DUI_PANEL_PORT:-}" ] || { echo "FAIL: port missing"; exit 1; }

        # No default admin in the DB.
        /usr/local/d-ui/d-ui setting -show | grep -q "hasDefaultCredential: false" \
            || { echo "FAIL: hasDefaultCredential is not false"; exit 1; }

        echo "--- verifying the panel serves HTTP ---"
        cd /usr/local/d-ui
        ./d-ui > /tmp/dui.log 2>&1 &
        xpid=$!
        for _ in $(seq 1 15); do
            code=$(curl -s -o /dev/null -w "%{http_code}" \
                "http://127.0.0.1:${DUI_PANEL_PORT}/${DUI_WEB_BASE_PATH}/" 2>/dev/null || true)
        ...
        case "${code:-}" in
            200|301|302|307|308) : ;;
            *) echo "FAIL: panel did not serve (status ${code:-none})"; tail -n 30 /tmp/dui.log; exit 1 ;;
        esac

        echo "SMOKE_PASS: user=$DUI_USERNAME port=$DUI_PANEL_PORT path=$DUI_WEB_BASE_PATH"
    '
