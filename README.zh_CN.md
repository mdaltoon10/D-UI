...
<p align="center">
  <picture>
    ...
    <img alt="d-ui" src="./media/d-ui-light.png">
  </picture>
</p>
...
若要安装特定版本，请在命令后附加对应的标签（例如 `v1.0.1`）：
...
设置 `DUI_NONINTERACTIVE=1`（或在无 TTY 的情况下通过管道传入），它就会全程
零提示地完成端到端安装，生成随机凭据并写入
`/etc/d-ui/install-result.env`。请参阅 [`deploy/`](deploy/)：
...
运行时通过环境变量选择后端（安装程序会为您写入 `/etc/default/d-ui`）：
...
# 然后在 /etc/default/d-ui 中设置 DUI_DB_TYPE 和 DUI_DB_DSN 并重启：
...
默认的 `docker compose up -d` 仍使用 SQLite。若要使用捆绑的 PostgreSQL 服务运行，请取消注释 `docker-compose.yml` 中的两行 `DUI_DB_*` 环境变量，并使用该 profile 启动：
...
| 变量 | 说明 | 默认值 |
| --- | --- | --- |
| `DUI_DB_TYPE` | 数据库后端：`sqlite` 或 `postgres` | `sqlite` |
| `DUI_DB_DSN` | PostgreSQL 连接字符串（当 `DUI_DB_TYPE=postgres` 时） | — |
| `DUI_DB_FOLDER` | SQLite 数据库文件所在目录 | `/etc/d-ui` |
| `DUI_DB_MAX_OPEN_CONNS` | 最大打开连接数（PostgreSQL 连接池） | — |
| `DUI_DB_MAX_IDLE_CONNS` | 最大空闲连接数（PostgreSQL 连接池） | — |
| `DUI_INIT_WEB_BASE_PATH` | Web 面板的初始 URI 路径 | `/` |
| `DUI_ENABLE_FAIL2BAN` | 启用基于 Fail2ban 的 IP 限制 | `true` |
| `DUI_LOG_LEVEL` | 日志级别（`debug`、`info`、`warning`、`error`） | `info` |
| `DUI_DEBUG` | 启用调试模式 | `false` |
| `DUI_TUNNEL_HEALTH_MONITOR` | 启用隧道健康监控（探测某个 URL，在连续多次失败后重启 xray；重启会断开所有客户端） | `false` |
| `DUI_TUNNEL_HEALTH_PROXY` | 探测请求所经过的代理；将其指向本地 xray 入站，使探测能够测试隧道（例如 `socks5://127.0.0.1:1080`）。留空表示探测仅检查主机连通性 | — |
| `DUI_TUNNEL_HEALTH_URL` | 用于检测隧道健康状况的探测 URL | `https://www.cloudflare.com/cdn-cgi/trace` |
| `DUI_TUNNEL_HEALTH_INTERVAL` | 两次探测之间的间隔 | `30s` |
| `DUI_TUNNEL_HEALTH_TIMEOUT` | 单次探测的超时时间 | `10s` |
| `DUI_TUNNEL_HEALTH_FAILURES` | 触发重启前的连续失败次数 | `3` |
| `DUI_TUNNEL_HEALTH_COOLDOWN` | 两次连续重启之间的最小间隔 | `5m` |
...
- [terraform-provider-d-ui](https://github.com/batonogov/terraform-provider-threexui) (许可证: **MIT**): _使用 Terraform / OpenTofu 通过代码管理入站、客户端、面板设置和 Xray 配置。_

## 支持项目

**如果这个项目对您有帮助，您可以给它一个**:star2:

<a href="https://www.buymeacoffee.com/mDaltoon" target="_blank">
<img src="./media/default-yellow.png" alt="Buy Me A Coffee" style="height: 70px !important;width: 277px !important;" >
</a>

</br>
<a href="https://nowpayments.io/donation/hmDaltoon" target="_blank" rel="noreferrer noopener">
   <img src="./media/donation-button-black.svg" alt="Crypto donation button by NOWPayments">
</a>

## 随时间变化的星标数

[![Stargazers over time](https://starchart.cc/mdaltoon10/D-UI.svg?variant=adaptive)](https://starchart.cc/mdaltoon10/D-UI)
