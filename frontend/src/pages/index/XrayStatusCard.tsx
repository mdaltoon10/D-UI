...
export default function XrayStatusCard({
  ...
  const actions = [
    ...
    ...(accessLogEnable
      ? [
          <Space
            ...
            role="button"
            tabIndex={0}
            aria-label={t('pages.index.accessLogs')}
            onClick={onOpenXrayLogs}
            onKeyDown={activateOnKey(onOpenXrayLogs)}
          >
            <BarsOutlined />
    ...
    <Space
      ...
      role="button"
      tabIndex={0}
      aria-label={t('pages.index.stopXray')}
      onClick={onStopXray}
      onKeyDown={activateOnKey(onStopXray)}
    >
      <PoweroffOutlined />
    ...
    <Space
      ...
      role="button"
      tabIndex={0}
      aria-label={t('pages.index.restartXray')}
      onClick={onRestartXray}
      onKeyDown={activateOnKey(onRestartXray)}
    >
      <ReloadOutlined />
    ...
    <Space
      ...
      onKeyDown={activateOnKey(onOpenVersionSwitch)}
    >
      <ToolOutlined />
      {!isMobile && (
        <span>
