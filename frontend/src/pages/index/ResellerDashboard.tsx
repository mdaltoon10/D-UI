...
export default function ResellerDashboard({
  ...
  const { data: clientsData } = useQuery({
    ...
    queryFn: async () => {
      ...
      }>('/panel/api/clients/list/paged?page=1&pageSize=1000', undefined, { silent: true });
      return msg?.obj;
    },
    staleTime: 10000,
  ...
  const { data: onlineList } = useQuery({
    ...
    queryFn: async () => {
      const msg = await HttpUtil.post<string[]>('/panel/api/clients/onlines', undefined, {
        silent: true,
      });
  ...
  if (!isUnlimitedTime) {
    ...
    if (totalDays <= 0 || daysRemaining > totalDays) {
      totalDays = Math.max(
        daysRemaining,
        Math.ceil((expiryTime - (createdAt || now - 86400000)) / 86400000),
      );
    }
  ...
  const clientLimitPercent = isUnlimitedClient
    ? 100
    : Math.min(100, (totalCount / clientLimit) * 100);
        ...
        <div className="reseller-quota-grid">
          ...
          <div className="reseller-quota-card">
            ...
            <div className="quota-details">
              <span>
                {tr.used}{' '}
                <b className="quota-val-highlight">{SizeFormatter.sizeFormat(volumeUsed)}</b>
              </span>
              <span>
                {tr.total}{' '}
                <b>
                  {isUnlimitedVolume
                    ? tr.unlimited
                    : formatTrafficQuota(adminInfo?.volumeGB, tr.unlimited)}
                </b>
              </span>
            </div>
          </div>
          ...
          <div className="reseller-quota-card">
            ...
            <div className="quota-details">
              <span>
                {tr.status}{' '}
                <b
                  className="quota-val-highlight"
                  style={{ color: daysRemaining <= 3 && !isUnlimitedTime ? '#ef4444' : '#00b4d8' }}
                >
                  {isUnlimitedTime ? tr.permanent : daysRemaining > 0 ? tr.active : tr.expired}
                </b>
              </span>
              <span>
                {tr.expiry}{' '}
                <b>{isUnlimitedTime ? tr.noExpiry : IntlUtil.formatDate(expiryTime, datepicker)}</b>
              </span>
            </div>
          </div>
          ...
          <div className="reseller-quota-card">
            ...
            <Progress
              ...
              strokeColor={{
                '0%': '#a855f7',
                '100%':
                  clientLimitPercent >= 90
                    ? '#ef4444'
                    : clientLimitPercent >= 75
                      ? '#f59e0b'
                      : '#a855f7',
              }}
              trailColor="rgba(255, 255, 255, 0.08)"
            ...
            <div className="quota-details">
              <span>
                {tr.used} <b className="quota-val-highlight">{totalCount}</b>
              </span>
              <span>
                {tr.total}{' '}
                <b>{isUnlimitedClient ? tr.unlimited : `${clientLimit} ${tr.clientCount}`}</b>
              </span>
            </div>
          </div>
        </div>
