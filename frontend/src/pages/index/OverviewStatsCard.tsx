...
export default function OverviewStatsCard({ isMobile: _isMobile }: OverviewStatsCardProps) {
  ...
  const { data: adminsData } = useQuery({
    ...
    queryFn: async () => {
      const msg = await HttpUtil.get<Array<{ enable?: boolean; expiryTime?: number }>>(
  ...
  const { data: clientsData } = useQuery({
    ...
    queryFn: async () => {
      ...
      }>('/panel/api/clients/list/paged?page=1&pageSize=10000&createdBy=all', undefined, {
        silent: true,
      });
      return msg?.obj;
    },
  ...
  const { data: groupsData } = useQuery({
    ...
    queryFn: async () => {
      const msg = await HttpUtil.get<unknown[]>('/panel/api/clients/groups', undefined, {
        silent: true,
      });
      return Array.isArray(msg?.obj) ? msg.obj : [];
    },
