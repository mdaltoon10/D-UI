          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            {editingAdmin && (
              <Popconfirm
                ...
                description={isFa ? 'ترافیک مصرفی این همکار ریست می‌شود (ترافیک کلاینت‌ها تغییر نمی‌کند). آیا اطمینان دارید؟' : 'Traffic for this admin will be reset (Clients traffic remains unchanged). Are you sure?'}
                okText={isFa ? 'بله' : 'Yes'}
                cancelText={dict.btnCancel}
                onConfirm={() => handleResetTraffic(editingAdmin.id)}
              >
                <Button color="danger" variant="filled" icon={<RetweetOutlined />} loading={resettingTrafficId === editingAdmin.id}>
            ...
            <div style={{ marginInlineStart: 'auto', display: 'flex', gap: 8 }}>
              ...
              <Button type="primary" onClick={handleFormSubmit}>{dict.btnSubmit}</Button>
            </div>
          </div>
        }
