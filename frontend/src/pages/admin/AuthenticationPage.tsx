...
export default function AuthenticationPage() {
  ...
  const handleUpdateProfile = async () => {
    if (!currentPassword) {
      messageApi.error(
        isFa ? 'لطفا رمز عبور فعلی خود را وارد کنید' : 'Please enter your current password',
      );
      return;
    }
    ...
    setLoading(true);
    try {
      const res = await HttpUtil.post<ResellerAdmin>(
        '/panel/api/admins/selfUpdate',
        {
      ...
      if (res?.success) {
        ...
        messageApi.error(
          res?.msg || (isFa ? 'خطا در بروزرسانی اطلاعات' : 'Failed to update profile'),
        );
      }
    } catch (err) {
      console.error(err);
          ...
          >
            <div style={{ marginBottom: 20 }}>
              ...
              <Typography.Text type="secondary">
                {isFa
                  ? 'اطلاعات کاربری و تنظیمات امنیتی پنل خود را مدیریت کنید'
                  : 'Manage your credentials and panel security settings'}
              </Typography.Text>
            </div>
                        ...
                        <Form layout="vertical">
                          ...
                          <Form.Item label={isFa ? 'نام کاربری جدید' : 'New Username'}>
                            <Input
                              ...
                              prefix={<UserOutlined style={{ opacity: 0.5 }} />}
                              placeholder={
                                isFa ? 'نام کاربری جدید را وارد کنید' : 'Enter new username'
                              }
                              style={{ borderRadius: 6 }}
                            />
                          </Form.Item>
