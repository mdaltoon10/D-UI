...
function AdminClientsSubList({
          ...
          const next = prev.map((item) => {
            ...
            return {
              ...
              traffic: {
                ...
                enable:
                  typeof upd.enable === 'boolean' ? upd.enable : (item.traffic?.enable ?? true),
                expiryTime:
                  typeof upd.expiryTime === 'number'
                    ? upd.expiryTime
                    : item.traffic?.expiryTime || 0,
              },
            };
          });
  ...
  const fetchClients = useCallback(
    async (silentLoad?: boolean | React.SyntheticEvent | unknown) => {
  ...
  const onSave = async (payload: any, meta: any) => {
    if (!meta.isEdit) {
      if (payload.client) {
  ...
  const clientBucket = useCallback(
    (row: ClientSlim | null | undefined): Bucket | null => {
      if (!row) return null;
    ...
    () => [
          ...
          <Space size={4}>
            ...
            <Tooltip title={t('delete')}>
              <Popconfirm
                title={
                  isFa
                    ? 'آیا از حذف این کلاینت مطمئن هستید؟'
      ...
      },
      {
        ...
        render: (_v, record) => (
          <div className="email-cell">
            ...
            {record.comment && (
              <span className="sub" title={record.comment}>
                {record.comment}
              </span>
            )}
          </div>
      ...
      },
      {
        ...
        render: (_v, record) => {
          ...
          const online =
            (Array.isArray(onlines) && onlines.includes(record.email)) ||
            speed.speedUp > 0 ||
            speed.speedDown > 0;
          return (
            <ClientTrafficCell
              ...
              return (
                <div key={row.email} className="client-card">
                  ...
                  {(() => {
                    ...
                    const online =
                      (Array.isArray(onlines) && onlines.includes(row.email)) ||
                      speed.speedUp > 0 ||
                      speed.speedDown > 0;
                    return (
                      <ClientTrafficCell
...
export default function ClientsAdminPage() {
  ...
  const fetchSettings = async () => {
    const res = await HttpUtil.post('/panel/api/setting/defaultSettings', undefined, {
      silent: true,
    });
    if (res.success && res.obj) {
      if (res.obj.subURI) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (record: ResellerAdmin) => {
        ...
        return <span style={{ fontWeight: 600 }}>{record.volumeGB} GB</span>;
      },
    },
    {
        ...
        <Layout className="content-shell">
          <Layout.Content className="content-area" style={{ padding: isMobile ? '8px' : '16px' }}>
            ...
            <Card
              title={
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  <span style={{ fontSize: isMobile ? '1.1rem' : '1.2rem', fontWeight: 800 }}>
                    {dict.title}
                  </span>
                  {!isMobile && (
                    <span
                    ...
                    >
                      ...
                      >
                        ...
                        <div style={{ display: 'flex', gap: 6 }}>
                          <Tag
                            color={admin.enable ? 'success' : 'error'}
                      ...
                      >
                        <div style={{ color: 'var(--ant-color-text-secondary)' }}>
                          {dict.colClientsCount}:{' '}
                          <Tag color="cyan" style={{ borderRadius: 4, margin: 0 }}>
                        ...
                        <div style={{ color: 'var(--ant-color-text-secondary)' }}>
                          ...
                          <span style={{ fontWeight: 600 }}>
                            {admin.volumeGB > 0 ? `${admin.volumeGB} GB` : dict.unlimited}
                          </span>
                        </div>
                        <div style={{ color: 'var(--ant-color-text-secondary)' }}>
                          {dict.colTrafficUsed}:{' '}
                          <span style={{ fontWeight: 600 }}>
                            {SizeFormatter.sizeFormat(admin.trafficUsedBytes || 0)}
                          </span>
                        </div>
                        <div style={{ color: 'var(--ant-color-text-secondary)' }}>
                          {dict.colExpiry}:{' '}
                          <span style={{ fontWeight: 600 }}>
                            {admin.expiryTime
                              ? new Date(admin.expiryTime).toLocaleDateString(
                                  isFa ? 'fa-IR' : 'en-US',
                                )
                              : dict.never}
                          </span>
