...
function AdminClientsSubList({
  ...
  }, [t, resetTraffic, fetchClients, onClientChange]);
  ...
  }, [remove, fetchClients, onClientChange, t]);
  ...
  const onToggleEnable = useCallback(async (row: any, next: boolean) => {
    ...
    try {
      ...
      if (msg?.success) {
        message.success(next ? dict.toastEnabled : dict.toastDisabled);
        fetchClients();
      } else {
        message.error(msg?.msg || t('somethingWentWrong'));
      }
  ...
  }, [setEnable, dict.toastEnabled, dict.toastDisabled, fetchClients, t]);
