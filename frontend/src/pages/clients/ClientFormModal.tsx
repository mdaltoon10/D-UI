...
export default function ClientFormModal({
  ...
  const isResellerOrHasAdminInbounds = !!isReseller || !!(adminInbounds && adminInbounds.length > 0);
  ...
  useEffect(() => {
    ...
    if (isEdit && client) {
      ...
      const next: FormState = {
        ...
        inboundIds: effectiveInboundIds ? [...effectiveInboundIds] : (Array.isArray(attachedIds) ? [...attachedIds] : []),
  ...
  async function onSubmit() {
    const finalInboundIds = effectiveInboundIds ? effectiveInboundIds : form.inboundIds;
