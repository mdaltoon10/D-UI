...
import { CodeOutlined, EyeInvisibleOutlined, PlusOutlined, DeleteOutlined } from '@ant-design/icons';
...
export const HiddenInfrastructureModal: React.FC<HiddenInfrastructureModalProps> = ({ open, onClose }) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'value',
      key: 'value',
      render: (val: string) => <Typography.Text code><EyeInvisibleOutlined /> {val}</Typography.Text>,
    },
    {
      title: 'Date Added',
      dataIndex: 'createdAt',
    ...
    },
    {
      ...
      render: (_: unknown, record: HiddenItem) => (
        <Button danger icon={<DeleteOutlined />} size="small" onClick={() => handleDeleteItem(record.id)} />
      ),
    },
  ];
  ...
  return (
    ...
    >
      <Typography.Paragraph type="secondary" style={{ marginBottom: 16 }}>
        Hidden Infrastructure allows internal resources (inbound remarks, outbound tags, balancers, client emails) to be hidden from normal views and subscription links without altering actual runtime behavior.
              ...
              >
                <div style={{ maxHeight: '300px', overflowY: 'auto', marginBottom: 12 }}>
                  {terminalLogs.map((log, idx) => (
                    <div key={idx} style={{ color: log.startsWith('-->') ? '#7ee787' : log.startsWith('===') ? '#ffa657' : '#c9d1d9' }}>
                      {log}
                    </div>
                  ))}
                </div>
