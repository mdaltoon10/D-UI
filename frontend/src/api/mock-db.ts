...
const DEFAULT_NODES: Node[] = [
  {
    id: 1,
    remark: 'Daltoon Germany Primary',
    address: 'de.daltoon.com',
    port: 2053,
    enable: true,
    status: 'online',
  },
  ...
  inbounds.forEach((ib) => {
    ...
    try {
      ...
      if (clients.length > 0) {
        ...
        clients.forEach(
          (c: { email: string; enable?: boolean; totalGB?: number; expiryTime?: number }) => {
            if (!c.enable) return;
...
export function handleMockRequest(
  ...
  _method: string,
  requestData: unknown,
): { success: boolean; msg?: string; obj?: unknown } | null {
  ...
  if (path === '/panel/api/inbounds/add') {
    ...
    const newInbound: Inbound = {
      ...
      settings:
        typeof body.settings === 'object' ? JSON.stringify(body.settings) : body.settings || '{}',
      streamSettings:
        typeof body.streamSettings === 'object'
          ? JSON.stringify(body.streamSettings)
          : body.streamSettings || '{}',
      tag: body.tag || `inbound-${newId}`,
      sniffing:
        typeof body.sniffing === 'object' ? JSON.stringify(body.sniffing) : body.sniffing || '{}',
      clientStats: [],
  ...
  if (path.startsWith('/panel/api/inbounds/update/')) {
    ...
    inbounds[inboundIndex] = {
      ...
      streamSettings:
        typeof body.streamSettings === 'object'
          ? JSON.stringify(body.streamSettings)
          : (body.streamSettings ?? existing.streamSettings),
      sniffing:
        typeof body.sniffing === 'object'
          ? JSON.stringify(body.sniffing)
          : (body.sniffing ?? existing.sniffing),
      ...
      try {
        ...
        if (idx !== -1) {
          ...
          clients[idx] = {
            ...
            downloadLimit:
              body.downloadLimit ?? body.downloadMbps ?? c.downloadLimit ?? c.downloadMbps,
            uploadMbps: body.uploadMbps ?? body.uploadLimit ?? c.uploadMbps ?? c.uploadLimit,
            downloadMbps:
              body.downloadMbps ?? body.downloadLimit ?? c.downloadMbps ?? c.downloadLimit,
      ...
      try {
        ...
        if (settings.clients) {
          settings.clients = settings.clients.filter(
            (c: InboundClient) => !emails.includes(c.email),
          );
          ib.settings = JSON.stringify(settings);
        }
        if (ib.clientStats) {
  ...
  if (path === '/panel/api/admins/delete') {
    ...
    if (adminToDelete) {
      ...
      inbounds.forEach((ib) => {
        try {
          ...
          if (settings.clients) {
            ...
            settings.clients = settings.clients.filter(
              (c: InboundClient) => c.createdBy !== username,
            );
            ib.settings = JSON.stringify(settings);

            if (ib.clientStats && emailsToDelete.length > 0) {
              ib.clientStats = ib.clientStats.filter(
                (st: { email: string }) => !emailsToDelete.includes(st.email),
              );
            }
          }
        } catch {}
      ...
      admins = admins.filter(
        (a) => a.id !== adminToDelete.id && a.username !== adminToDelete.username,
      );
    } else {
  ...
  if (path === '/panel/api/admins/self') {
    ...
    try {
      ...
      const currentAdmin = admins.find(
        (a) => a.username === currentSession.username || a.webPath === currentSession.webPath,
      );
      if (currentAdmin) {
      ...
      if (idx !== -1) {
        ...
        if (body.newUsername) {
          const usernameExists = admins.some(
            (a, i) => i !== idx && a.username === body.newUsername,
          );
          if (usernameExists) {
