...
function resolveDBPath() {
  const envFolder = process.env.DUI_DB_FOLDER;
  if (envFolder) {
    const abs = path.isAbsolute(envFolder)
      ? envFolder
      : path.resolve(__dirname, '..', envFolder);
    return path.join(abs, 'd-ui.db');
  }
  const repoSubDB = path.resolve(__dirname, '..', 'd-ui', 'd-ui.db');
  if (fs.existsSync(repoSubDB)) return repoSubDB;
  const repoDB = path.resolve(__dirname, '..', 'd-ui.db');
  if (fs.existsSync(repoDB)) return repoDB;
  return '/etc/d-ui/d-ui.db';
}
...
function bypassMigratedRoute(req) {
  ...
  if (url === basePath) return '/index.html';
