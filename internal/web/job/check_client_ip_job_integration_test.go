...
// d-ui logger must be initialised once before any code path that can
...
func setupIntegrationDB(t *testing.T) {
	...
	t.Setenv("DUI_LOG_FOLDER", logDir)
...
func TestRun_NoOpWhenOnlineApiUnavailable(t *testing.T) {
	...
	if info, err := os.Stat(readIpLimitLogPath()); err == nil && info.Size() > 0 {
		...
		t.Fatalf("duiipl.log should be empty when Run no-ops, got:\n%s", body)
	}
	var count int64
...
func TestUpdateInboundClientIps_LiveIpNotBannedByStillFreshHistoricals(t *testing.T) {
	...
	// duiipl.log must not contain a ban line.
	if info, err := os.Stat(readIpLimitLogPath()); err == nil && info.Size() > 0 {
		body, _ := os.ReadFile(readIpLimitLogPath())
		t.Fatalf("duiipl.log should be empty when no ips are banned, got:\n%s", body)
	}
}
...
func TestUpdateInboundClientIps_ExcessLiveIpIsStillBanned(t *testing.T) {
	...
	// duiipl.log must contain the ban line in the exact fail2ban format.
	body, err := os.ReadFile(readIpLimitLogPath())
	if err != nil {
		t.Fatalf("read duiipl.log: %v", err)
	}
	wantSubstr := "[LIMIT_IP] Email = pr4091-abuse || Disconnecting OLD IP = 10.1.0.1"
	if !contains(string(body), wantSubstr) {
		t.Fatalf("duiipl.log missing expected ban line %q\nfull log:\n%s", wantSubstr, body)
	}
}
...
func TestProcessObserved_CollectsIpsWithoutLimit(t *testing.T) {
	...
	if info, err := os.Stat(readIpLimitLogPath()); err == nil && info.Size() > 0 {
		body, _ := os.ReadFile(readIpLimitLogPath())
		t.Fatalf("duiipl.log should be empty with no limit set, got:\n%s", body)
	}
}
...
// readIpLimitLogPath reads the duiipl.log path the same way the job
...
func readIpLimitLogPath() string {
	folder := os.Getenv("DUI_LOG_FOLDER")
	if folder == "" {
		folder = filepath.Join(".", "log")
	}
	return filepath.Join(folder, "duiipl.log")
}
