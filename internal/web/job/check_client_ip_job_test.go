...
func toTimestampMap(m map[string]IPWithTimestamp) map[string]int64 {
...
func TestMergeClientIps_EvictsStaleOldEntries(t *testing.T) {
	...
	got := toTimestampMap(mergeClientIps(old, new, 1000, false))
...
func TestMergeClientIps_KeepsFreshOldEntriesUnchanged(t *testing.T) {
	...
	got := toTimestampMap(mergeClientIps(old, nil, 1000, false))
...
func TestMergeClientIps_PrefersLaterTimestampForSameIp(t *testing.T) {
	...
	if got["1.1.1.1"] != 1700 {
		t.Fatalf("expected latest timestamp 1700, got %v", got)
	}
}
...
func TestMergeClientIps_DropsStaleNewEntries(t *testing.T) {
	...
	got := toTimestampMap(mergeClientIps(nil, new, 1000, false))
...
func TestMergeClientIps_NoStaleCutoffStillWorks(t *testing.T) {
	...
	got := toTimestampMap(mergeClientIps(old, new, 0, false))
...
func TestMergeClientIps_LiveObservationsBypassStaleCutoff(t *testing.T) {
	...
	got := toTimestampMap(mergeClientIps(nil, new, 1000, true))
...
func TestMergeClientIps_LiveModeStillEvictsStaleOldEntries(t *testing.T) {
	...
	got := toTimestampMap(mergeClientIps(old, new, 1000, true))
...
func TestSelectIpsToBan(t *testing.T) {
	...
	// kick_oldest: over the limit: oldest connections are banned, newest keep the slots
	kept, banned := selectIpsToBan(live, 1, "kick_oldest")
	...
	// block_newest: over the limit: newest connections are banned, oldest keep the slots
	kept, banned = selectIpsToBan(live, 1, "block_newest")
	...
	if _, banned := selectIpsToBan(live, 0, "kick_oldest"); banned != nil {
...
func TestPartitionLiveIps_SingleLiveNotStarvedByStillFreshHistoricals(t *testing.T) {
	...
	ipMap := map[string]IPWithTimestamp{
		"A": {IP: "A", Timestamp: 1000},
		"B": {IP: "B", Timestamp: 1100},
		"C": {IP: "C", Timestamp: 1200},
		"D": {IP: "D", Timestamp: 2000},
	}
...
func TestPartitionLiveIps_ConcurrentLiveIpsSortedAscending(t *testing.T) {
	...
	ipMap := map[string]IPWithTimestamp{
		"A": {IP: "A", Timestamp: 5000},
		"B": {IP: "B", Timestamp: 5500},
	}
...
func TestCheckFail2BanInstalled_DisabledEnvSkipsClientProbe(t *testing.T) {
	t.Setenv("DUI_ENABLE_FAIL2BAN", "false")
	marker := fakeFail2BanClient(t)

	if (&CheckClientIpJob{}).checkFail2BanInstalled() {
		t.Fatal("fail2ban should be unavailable when DUI_ENABLE_FAIL2BAN=false")
	}
...
func TestCheckFail2BanInstalled_EmptyEnvSkipsClientProbe(t *testing.T) {
	t.Setenv("DUI_ENABLE_FAIL2BAN", "")
	marker := fakeFail2BanClient(t)

	if (&CheckClientIpJob{}).checkFail2BanInstalled() {
		t.Fatal("fail2ban should be unavailable when DUI_ENABLE_FAIL2BAN is empty")
	}
	if _, err := os.Stat(marker); !os.IsNotExist(err) {
		t.Fatalf("fail2ban-client should not have been executed, stat error: %v", err)
	}
...
func TestIsFail2BanEnabled_DefaultsToEnabledWhenUnset(t *testing.T) {
	...
	os.Unsetenv("DUI_ENABLE_FAIL2BAN")
	t.Cleanup(func() {
		if ok {
			os.Setenv("DUI_ENABLE_FAIL2BAN", value)
		} else {
			os.Unsetenv("DUI_ENABLE_FAIL2BAN")
		}
	})
	...
	if !isFail2BanEnabled() {
		t.Fatal("fail2ban should default to enabled when DUI_ENABLE_FAIL2BAN is unset")
	}
}

func TestCheckFail2BanInstalled_EnabledEnvProbesClient(t *testing.T) {
	t.Setenv("DUI_ENABLE_FAIL2BAN", "true")
	marker := fakeFail2BanClient(t)
