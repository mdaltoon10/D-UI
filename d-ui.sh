...
if [[ -f /.dockerenv ]] || [[ "${DUI_IN_DOCKER}" == "true" ]]; then
    running_in_docker="true"
fi

# Declare Variables
if [[ "${running_in_docker}" == "true" ]]; then
    dui_folder="${DUI_MAIN_FOLDER:=/app}"
else
    dui_folder="${DUI_MAIN_FOLDER:=/usr/local/d-ui}"
fi
dui_service="${DUI_SERVICE:=/etc/systemd/system}"
log_folder="${DUI_LOG_FOLDER:=/var/log/d-ui}"
mkdir -p "${log_folder}"
iplimit_log_path="${log_folder}/duiipl.log"
iplimit_banned_log_path="${log_folder}/duiipl-banned.log"
...
update() {
    confirm "This function will update all d-ui components to the latest version, and the data will not be lost. Do you want to continue?" "y"
    if [[ $? != 0 ]]; then
        LOGE "Cancelled"
        if [[ $# == 0 ]]; then
            before_show_menu
...
update_dev() {
    confirm "This will update d-ui to the latest DEV commit (the rolling 'dev-latest' build, not a stable release). Your data is preserved. Continue?" "y"
    if [[ $? != 0 ]]; then
        LOGE "Cancelled"
        if [[ $# == 0 ]]; then
            before_show_menu
...
replace_xui_script() {
    ...
    local temp_file="/usr/bin/d-ui-temp.$$"

    rm -f "$temp_file"
    if [[ "$use_if_modified_since" == "true" ]]; then
        curl -fLRo "$temp_file" -z /usr/bin/d-ui "$url"
    else
...
dui_env_file_path() {
    case "${release}" in
        ubuntu | debian | armbian)
            echo "/etc/default/d-ui"
            ;;
        arch | manjaro | parch | alpine)
            echo "/etc/conf.d/d-ui"
            ;;
        *)
            echo "/etc/sysconfig/d-ui"
            ;;
    esac
}
...
uninstall() {
    ...
    if [[ $release == "alpine" ]]; then
        ...
        rc-update del d-ui
        rm /etc/init.d/d-ui -f
    else
        systemctl stop d-ui
        systemctl disable d-ui
        rm ${dui_service}/d-ui.service -f
    ...
    local db_env_file
    db_env_file="$(dui_env_file_path)"
    if [[ -r "$db_env_file" ]] && grep -q '^DUI_DB_TYPE=postgres' "$db_env_file"; then
...
reset_user() {
    ...
    if [[ $twoFactorConfirm != "y" && $twoFactorConfirm != "Y" ]]; then
        ${dui_folder}/d-ui setting -username "${config_account}" -password "${config_password}" > /dev/null 2>&1
    else
        ${dui_folder}/d-ui setting -username "${config_account}" -password "${config_password}" -resetTwoFactor=true > /dev/null 2>&1
        echo -e "Two factor authentication has been disabled."
    fi

    echo -e "Panel login username has been reset to: ${green} ${config_account} ${plain}"
    echo -e "Panel login password has been reset to: ${green} ${config_password} ${plain}"
    echo -e "${green} Please use the new login username and password to access the Daltoon-UI panel. Also remember them! ${plain}"
    confirm_restart
}
...
reset_webbasepath() {
    ...
    ${dui_folder}/d-ui setting -webBasePath "${config_webBasePath}" > /dev/null 2>&1
...
reset_config() {
    ...
    ${dui_folder}/d-ui setting -reset
    echo -e "All panel settings have been reset to default."
    restart
}

check_config() {
    local info=$(${dui_folder}/d-ui setting -show true)
    if [[ $? != 0 ]]; then
    ...
    if [[ -r "$db_env_file" ]] && grep -q '^DUI_DB_TYPE=postgres' "$db_env_file"; then
        local dsn
        dsn="$(grep -E '^DUI_DB_DSN=' "$db_env_file" | head -1 | cut -d= -f2-)"
        local dsn_safe
        dsn_safe="$(echo "$dsn" | sed -E 's|(://[^:/@]+:)[^@]+@|\1****@|')"
        echo -e "${green}Database: PostgreSQL — ${dsn_safe}${plain}"
    else
        echo -e "${green}Database: SQLite (/etc/d-ui/d-ui.db)${plain}"
    fi
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        ${dui_folder}/d-ui setting -port ${port}
...
start() {
    ...
    if [[ $? == 0 ]]; then
        ...
        check_status
        if [[ $? == 0 ]]; then
            LOGI "d-ui Started Successfully"
        else
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        check_status
        if [[ $? == 1 ]]; then
            LOGI "d-ui and xray stopped successfully"
        else
...
restart() {
    if [[ "${running_in_docker}" == "true" ]]; then
        ...
        check_status
        if [[ $? == 0 ]]; then
            LOGI "d-ui and xray Restarted successfully"
        else
...
restart_xray() {
    ...
    if [[ $release == "alpine" ]]; then
        rc-service d-ui reload
    else
        systemctl reload d-ui
    fi
    LOGI "xray-core Restart signal sent successfully, Please check the log information to confirm whether xray restarted successfully"
    sleep 2
    show_xray_status
...
status() {
    ...
    if [[ $release == "alpine" ]]; then
        rc-service d-ui status
    else
        systemctl status d-ui -l
    fi
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
...
enable() {
    ...
    if [[ $release == "alpine" ]]; then
        rc-update add d-ui default
    else
        systemctl enable d-ui
    fi
    if [[ $? == 0 ]]; then
        LOGI "d-ui Set to boot automatically on startup successfully"
    else
        LOGE "d-ui Failed to set Autostart"
    fi
...
disable() {
    ...
    if [[ $? == 0 ]]; then
        LOGI "d-ui Autostart Cancelled successfully"
    else
        LOGE "d-ui Failed to cancel autostart"
    fi
    ...
    if [[ $release == "alpine" ]]; then
        ...
        case "$choice" in
            0)
                ...
                ;;
            1)
                journalctl -u d-ui -e --no-pager -f -p debug
                if [[ $# == 0 ]]; then
                    before_show_menu
                fi
                ;;
...
disable_bbr() {
    ...
    if [ -f "/etc/sysctl.d/99-bbr-d-ui.conf" ]; then
        old_settings=$(head -1 /etc/sysctl.d/99-bbr-d-ui.conf | tr -d '#')
        ...
        rm /etc/sysctl.d/99-bbr-d-ui.conf
    else
...
enable_bbr() {
    ...
    if [ -d "/etc/sysctl.d/" ]; then
        ...
        sysctl -p /etc/sysctl.d/99-bbr-d-ui.conf
    else
...
dui_pid() {
    ps -ef 2> /dev/null | grep -F "${dui_folder}/d-ui" | grep -v grep | awk 'NR==1 {print $1}'
}
...
check_status() {
    ...
    if [[ $release == "alpine" ]]; then
        if [[ ! -f /etc/init.d/d-ui ]]; then
            return 2
        fi
        if [[ $(rc-service d-ui status | grep -F 'status: started' -c) == 1 ]]; then
            return 0
        else
            return 1
        fi
    else
        if [[ ! -f ${dui_service}/d-ui.service ]]; then
            return 2
        fi
        temp=$(systemctl status d-ui | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
check_enabled() {
    if [[ $release == "alpine" ]]; then
        if [[ $(rc-update show | grep -F 'd-ui' | grep default -c) == 1 ]]; then
            return 0
        else
            return 1
        fi
    else
        temp=$(systemctl is-enabled d-ui)
...
show_mtproto_status() {
    local cfg_dir="${dui_folder}/bin/mtproto"
    local cfgs=()
    if [[ -d "${cfg_dir}" ]]; then
...
install_firewall() {
    if ! command -v ufw &> /dev/null; then
        ...
        # Using force-confdef to prevent interactive prompts
        apt-get update
        apt-get install -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" -y ufw
    else
...
update_geofiles() {
    ...
    for dat in "${dat_files[@]}"; do
        ...
        remote_file="${dat%%_*}"
        local dest="${dui_folder}/bin/${dat}.dat"
...
ssl_cert_issue_main() {
    ...
    case "$choice" in
        ...
        4)
            ...
            local panel_cert=$(${dui_folder}/d-ui setting -getCert true | grep 'cert:' | awk -F': ' '{print $2}' | tr -d '[:space:]')
                    ...
                    if [[ -f "${webCertFile}" && -f "${webKeyFile}" ]]; then
                        ...
                        if command -v ~/.acme.sh/acme.sh &> /dev/null && ~/.acme.sh/acme.sh --list 2> /dev/null | awk '{print $1}' | grep -Fxq "${domain}"; then
                            ~/.acme.sh/acme.sh --installcert --force -d "${domain}" \
                                ...
                                --reloadcmd "d-ui restart" 2>&1 || true
...
ssl_cert_issue_for_ip() {
    ...
    local existing_webBasePath=$(${dui_folder}/d-ui setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    ...
    case "${release}" in
        ubuntu | debian | armbian)
            apt-get update > /dev/null 2>&1 && apt-get install -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" socat -y > /dev/null 2>&1
            ;;
    ...
    local reloadCmd="systemctl restart d-ui 2>/dev/null || rc-service d-ui restart 2>/dev/null"
...
ssl_cert_issue() {
    ...
    reloadCmd="d-ui restart"

    LOGI "Default --reloadcmd for ACME is: ${yellow}d-ui restart"
    LOGI "This command will run on every certificate issue and renew."
    read -rp "Would you like to modify --reloadcmd for ACME? (y/n): " setReloadcmd
    if [[ "$setReloadcmd" == "y" || "$setReloadcmd" == "Y" ]]; then
        echo -e "\n${green}\t1.${plain} Preset: systemctl reload nginx ; d-ui restart"
        echo -e "${green}\t2.${plain} Input your own command"
        echo -e "${green}\t0.${plain} Keep default reloadcmd"
        read -rp "Choose an option: " choice
        case "$choice" in
            1)
                LOGI "Reloadcmd is: systemctl reload nginx ; d-ui restart"
                reloadCmd="systemctl reload nginx ; d-ui restart"
                ;;
            2)
                LOGD "It's recommended to put d-ui restart at the end, so it won't raise an error if other services fails"
                read -rp "Please enter your reloadcmd (example: systemctl reload nginx ; d-ui restart): " reloadCmd
...
iplimit_main() {
    ...
    case "$choice" in
        ...
        1)
            ...
            ;;
        2)
            read -rp "Please enter new Ban Duration in Minutes [default 1]: " NUM
            if [[ $NUM =~ ^[0-9]+$ ]]; then
        ...
        3)
            ...
            if [[ $? == 0 ]]; then
                fail2ban-client reload --restart --unban dui-ipl
        ...
        5)
            ...
            if [[ $ban_ip =~ $ipv4_regex || $ban_ip =~ $ipv6_regex ]]; then
                fail2ban-client set dui-ipl banip "$ban_ip"
        ...
        6)
            ...
            if [[ $unban_ip =~ $ipv4_regex || $unban_ip =~ $ipv6_regex ]]; then
                fail2ban-client set dui-ipl unbanip "$unban_ip"
...
setup_fail2ban_iplimit() {
    ...
    if [[ -n "${DUI_ENABLE_FAIL2BAN+x}" && "${DUI_ENABLE_FAIL2BAN}" != "true" ]]; then
        echo -e "${yellow}DUI_ENABLE_FAIL2BAN=${DUI_ENABLE_FAIL2BAN}, skipping Fail2ban setup.${plain}\n"
        return 0
    fi
    ...
    if ! command -v fail2ban-client &> /dev/null; then
        ...
        # even though our own dui-ipl jail uses iptables. Bundling the binary
        # at install time prevents that confusing log spam for new installs.
        case "${release}" in
            ubuntu)
                apt-get update
                if [[ "${os_version}" -ge 2400 ]]; then
                    apt-get install -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" python3-pip -y
                    python3 -m pip install pyasynchat --break-system-packages
                fi
                apt-get install -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" fail2ban nftables -y
                ;;
            debian)
                apt-get update
                if [ "$os_version" -ge 12 ]; then
                    apt-get install -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" -y python3-systemd
                fi
                apt-get install -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" -y fail2ban nftables
                ;;
            armbian)
                apt-get update && apt-get install -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" fail2ban nftables -y
                ;;
            fedora | amzn | virtuozzo | rhel | almalinux | rocky | ol)
...
# setup_fail2ban_iplimit directly via `d-ui setup-fail2ban`.
install_iplimit() {
    setup_fail2ban_iplimit
    before_show_menu
}
...
remove_iplimit() {
    ...
    case "$num" in
        1)
            rm -f /etc/fail2ban/filter.d/dui-ipl.conf
            rm -f /etc/fail2ban/action.d/dui-ipl.conf
            rm -f /etc/fail2ban/jail.d/dui-ipl.conf
            if [[ $release == "alpine" ]]; then
                rc-service fail2ban restart
            else
                systemctl restart fail2ban
...
show_banlog() {
    ...
    if [[ -f "$system_log" ]]; then
        ...
        grep "dui-ipl" "$system_log" | grep -E "Ban|Unban" | tail -n 10 || echo -e "${yellow}No recent system ban activities found${plain}"
        echo ""
    fi

    if [[ -f "${iplimit_banned_log_path}" ]]; then
        echo -e "${green}DUI-IPL ban log entries:${plain}"
    ...
    fail2ban-client status dui-ipl || echo -e "${yellow}Unable to get jail status${plain}"
}
...
create_iplimit_jails() {
    ...
    cat << EOF > /etc/fail2ban/jail.d/dui-ipl.conf
[dui-ipl]
enabled=true
backend=auto
filter=dui-ipl
...
EOF
    ...
    cat << EOF > /etc/fail2ban/filter.d/dui-ipl.conf
[Definition]
...
EOF
    ...
    local panel_port
    panel_port=$(${dui_folder}/d-ui setting -show true 2>/dev/null | grep -Eo 'port: .+' | awk '{print $2}')
    local exempt_ports="${ssh_ports}"
    [[ -n "${panel_port}" ]] && exempt_ports="${exempt_ports},${panel_port}"

    cat << EOF > /etc/fail2ban/action.d/dui-ipl.conf
[INCLUDES]
...
EOF
    ...
    cat << EOF > /etc/fail2ban/action.d/dui-ipl-v6.conf
[INCLUDES]
...
actionban = <ip6tables> -I f2b-<name> 1 -s <ip> -p tcp -m multiport ! --dports <exemptports> -j <blocktype>
            ...
            echo "\$(date +"%%Y/%%m/%%d %%H:%%M:%%S")   BAN_V6   [Email] = <F-USER> [IP] = <ip> banned for <bantime> seconds." >> ${iplimit_banned_log_path}
...
actionunban = <ip6tables> -D f2b-<name> -s <ip> -p tcp -m multiport ! --dports <exemptports> -j <blocktype>
              ...
              echo "\$(date +"%%Y/%%m/%%d %%H:%%M:%%S")   UNBAN_V6   [Email] = <F-USER> [IP] = <ip> unbanned." >> ${iplimit_banned_log_path}
...
iplimit_remove_conflicts() {
    ...
    for file in "${jail_files[@]}"; do
        # Check for [dui-ipl] config in jail file then remove it
        if test -f "${file}" && grep -qw 'dui-ipl' ${file}; then
            sed -i "/\[dui-ipl\]/,/^$/d" ${file}
            echo -e "${yellow}Removing conflicts of [dui-ipl] in jail (${file})!${plain}\n"
        fi
    done
}
...
SSH_port_forwarding() {
    ...
    local existing_webBasePath=$(${dui_folder}/d-ui setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    local existing_port=$(${dui_folder}/d-ui setting -show true | grep -Eo 'port: .+' | awk '{print $2}')
    local existing_listenIP=$(${dui_folder}/d-ui setting -getListen true | grep -Eo 'listenIP: .+' | awk '{print $2}')
    local existing_cert=$(${dui_folder}/d-ui setting -getCert true | grep -Eo 'cert: .+' | awk '{print $2}')
    local existing_key=$(${dui_folder}/d-ui setting -getCert true | grep -Eo 'key: .+' | awk '{print $2}')
    ...
    case "$num" in
        1)
            if [[ -z "$existing_listenIP" || "$existing_listenIP" == "0.0.0.0" ]]; then
                ...
                ${dui_folder}/d-ui setting -listenIP "${config_listenIP}" > /dev/null 2>&1
            ...
            ;;
        2)
            ${dui_folder}/d-ui setting -listenIP 0.0.0.0 > /dev/null 2>&1
...
# PostgreSQL service management (for panels configured with DUI_DB_TYPE=postgres).
...
# Installs a local PostgreSQL server and creates a dedicated dui user/database.
...
pg_install_local() {
    ...
    case "${release}" in
        ubuntu | debian | armbian)
            apt-get update >&2 && apt-get install -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" -y -q postgresql >&2 || return 1
            ;;
        fedora | amzn | virtuozzo | rhel | almalinux | rocky | ol)
...
pg_ensure_client() {
    ...
    case "${release}" in
        ubuntu | debian | armbian)
            apt-get update >&2 && apt-get install -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" -y -q postgresql-client >&2 || return 1
            ;;
        fedora | amzn | virtuozzo | rhel | almalinux | rocky | ol)
...
# Writes DUI_DB_TYPE/DUI_DB_DSN into the service env file, preserving other entries.
pg_write_env() {
    local dsn="$1" envfile
    envfile="$(dui_env_file_path)"
    install -d -m 755 "$(dirname "$envfile")"
    touch "$envfile"
    sed -i '/^DUI_DB_TYPE=/d; /^DUI_DB_DSN=/d' "$envfile"
    {
        echo "DUI_DB_TYPE=postgres"
        echo "DUI_DB_DSN=${dsn}"
    } >> "$envfile"
    chmod 600 "$envfile"
}
...
pg_install_server_action() {
    if postgresql_installed; then
        ...
        confirm "Run setup anyway (ensures the dui database/user exist)?" "n" || return 0
    fi
...
migrate_to_postgres() {
    ...
    if ! ${dui_folder}/d-ui migrate-db --dsn "$dsn"; then
    ...
    pg_write_env "$dsn"
    LOGI "Wrote database settings to $(dui_env_file_path) (DUI_DB_TYPE=postgres)."
...
postgresql_menu() {
    echo -e "${green}\t1.${plain} ${green}Install${plain} PostgreSQL (server + client + dui db)"
...
# file using the bundled d-ui binary. With no arguments it dumps the installed
# panel database; an optional second argument overrides the output path.
#   d-ui migrateDB [file.db|file.dump] [output]
migrate_db() {
    local input="$1" output="$2"
    local default_db="/etc/d-ui/d-ui.db"
    local bin="${dui_folder}/d-ui"

    [[ -z "$input" ]] && input="$default_db"

    if [[ ! -x "$bin" ]]; then
        LOGE "d-ui binary not found at ${bin}. Is the panel installed?"
        return 1
    fi

    if ! "$bin" migrate-db -h 2>&1 | grep -q -- '-dump'; then
        LOGE "This d-ui build does not support .db <-> .dump conversion yet."
        LOGE "Update the panel first (d-ui update) to a version with 'migrate-db --dump/--restore'."
        return 1
    fi

    if [[ ! -f "$input" ]]; then
        LOGE "Input file not found: ${input}"
        echo -e "Usage: ${green}d-ui migrateDB [file.db|file.dump] [output]${plain}"
        return 1
    fi
    ...
    if [[ "$mode" == "dump" ]]; then
        ...
        if [[ "$output" == "$default_db" ]] && check_status > /dev/null 2>&1; then
            LOGE "Refusing to restore into the live database (${default_db}) while d-ui is running."
            LOGE "Stop the panel first (d-ui stop) or choose a different output path."
            return 1
        fi
        if [[ -f "$output" ]]; then
            confirm "Output ${output} already exists and will be overwritten. Continue?" "n" || return 0
...
|  ${blue}d-ui restart-xray${plain}          - Restart Xray                     │
...
show_menu() {
    echo -e "
╔────────────────────────────────────────────────╗
│  ${green}Daltoon-UI Panel Management Script${plain}                │
