...
import (
	...
	"github.com/mdaltoon10/D-UI/v3/internal/sub"
	"github.com/mdaltoon10/D-UI/v3/internal/tunnelmonitor"
	"github.com/mdaltoon10/D-UI/v3/internal/util/crypto"
	"github.com/mdaltoon10/D-UI/v3/internal/util/sys"
	"github.com/mdaltoon10/D-UI/v3/internal/web"
	"github.com/mdaltoon10/D-UI/v3/internal/web/global"
...
// runWebServer initializes and starts the web server for the d-ui panel.
func runWebServer() {
	...
	if monitorCfg.Enabled {
		if monitorCfg.ProxyURL == "" {
			logger.Warning("Tunnel health monitor enabled without DUI_TUNNEL_HEALTH_PROXY: the probe measures host connectivity, not the xray tunnel, so failures will restart xray without fixing host network issues")
		}
...
// migrateDb performs database migration operations for the d-ui panel.
func migrateDb() {
...
// "d-ui setting" hit the same database backend as the panel. godotenv.Load does
...
// main is the entry point of the d-ui application.
...
func main() {
	...
	migrateDbCmd.StringVar(&migrateSrc, "src", "", "Source SQLite file (defaults to the configured d-ui.db)")
