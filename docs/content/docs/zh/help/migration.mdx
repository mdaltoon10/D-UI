---
title: 迁移
description: 从 x-ui 迁移到 Daltoon-UI、在服务器之间迁移，或在不同 Daltoon-UI 版本之间迁移。
icon: ArrowRightLeft
---
...
## 在不同 Daltoon-UI 版本之间迁移

在同一后端内，升级通常只需重新运行安装程序或拉取新镜像即可——面板会自行迁移其数据库。如果是跨**大版本**升级，请先阅读
[版本发布说明](https://github.com/mdaltoon10/Daltoon-UI/releases)
并做好备份。

## 从 x-ui 迁移

将旧版 x-ui 面板的数据库直接导入 Daltoon-UI **不受支持**——两者的数据库结构不同。请全新安装 Daltoon-UI 并重新创建入站/客户端，同时从旧面板逐一导出分享链接。
