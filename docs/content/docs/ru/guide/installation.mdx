...
bash <(curl -Ls https://raw.githubusercontent.com/mdaltoon10/D-UI/master/install.sh) v3.4.0
```
...
docker run -d \
  ...
  --name Daltoon-UI \
  ghcr.io/mdaltoon10/D-UI:latest
```
...
[страницы релизов](https://github.com/mdaltoon10/D-UI/releases), распаковать его и
...
```bash
XUI_NONINTERACTIVE=1 bash <(curl -Ls https://raw.githubusercontent.com/mdaltoon10/D-UI/master/install.sh)
```

В каталоге [`deploy/`](https://github.com/mdaltoon10/D-UI/tree/main/deploy)
