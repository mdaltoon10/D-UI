---
title: Установка
description: Установите Daltoon-UI с помощью официального скрипта (стабильная, фиксированная или dev-сборка), в автоматическом режиме / через cloud-init либо в Docker — и выберите SQLite или PostgreSQL.
icon: Download
---

Daltoon-UI работает на широком спектре дистрибутивов Linux — Ubuntu, Debian, Armbian,
...
bash <(curl -Ls https://raw.githubusercontent.com/mdaltoon10/Daltoon-UI/master/install.sh) v3.4.0
```
...
docker run -d \
  ...
  --name Daltoon-UI \
  ghcr.io/mdaltoon10/Daltoon-UI:latest
```
...
[страницы релизов](https://github.com/mdaltoon10/Daltoon-UI/releases), распаковать его и
...
```bash
XUI_NONINTERACTIVE=1 bash <(curl -Ls https://raw.githubusercontent.com/mdaltoon10/Daltoon-UI/master/install.sh)
```

В каталоге [`deploy/`](https://github.com/mdaltoon10/Daltoon-UI/tree/main/deploy)
