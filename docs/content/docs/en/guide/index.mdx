...
**D-UI** is a web control panel that sits on top of
...
<Mermaid
  chart={`
flowchart LR
  Admin["Admin browser"] -->|HTTPS panel| Panel["D-UI panel"]
...
D-UI is aimed at anyone running their own Xray server: from a single personal
...
<Callout type="info">
  D-UI is an enhanced fork of the original X-UI project, adding broader protocol
