...
description: Complete reference for D-UI's XUI_* environment variables — database, panel, logging, memory, and the tunnel health monitor.
icon: Variable
---

D-UI reads its runtime configuration from `XUI_*` environment variables. On a
