...
description: Shape egress in D-UI — WARP and NordVPN outbounds, outbound subscriptions (server pools), routing rules, and load balancers.
icon: Route
---

Inbounds accept clients; **outbounds** decide where their traffic goes next.
D-UI can route traffic through Cloudflare WARP, NordVPN, or arbitrary outbound
...
shapes D-UI uses: **VLESS is the flat form** (`address`/`port`/`id`/`flow`/
...
WARP lets your server egress through Cloudflare's network. D-UI can register a
...
From the panel's WARP controls, register an account. D-UI fills the outbound's
...
D-UI can fetch NordVPN (NordLynx/WireGuard) credentials from an access token (or
