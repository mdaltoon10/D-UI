...
description: Shape egress in Daltoon-UI — WARP and NordVPN outbounds, outbound subscriptions (server pools), routing rules, and load balancers.
icon: Route
---

Inbounds accept clients; **outbounds** decide where their traffic goes next.
Daltoon-UI can route traffic through Cloudflare WARP, NordVPN, or arbitrary outbound
...
shapes Daltoon-UI uses: **VLESS is the flat form** (`address`/`port`/`id`/`flow`/
...
WARP lets your server egress through Cloudflare's network. Daltoon-UI can register a
...
From the panel's WARP controls, register an account. Daltoon-UI fills the outbound's
...
Daltoon-UI can fetch NordVPN (NordLynx/WireGuard) credentials from an access token (or
