...
describe('buildScriptCommand', () => {
  ...
  it('pins a specific version by passing the tag to master install.sh', () => {
    ...
    expect(cmd).toBe(
      'bash <(curl -Ls https://raw.githubusercontent.com/mdaltoon10/Daltoon-UI/master/install.sh) v3.4.1',
    );
  });
  ...
  it('supports the rolling dev-latest build', () => {
    ...
    expect(cmd).toBe(
      'bash <(curl -Ls https://raw.githubusercontent.com/mdaltoon10/Daltoon-UI/master/install.sh) dev-latest',
    );
  });
});
...
describe('buildDockerRun', () => {
  it('reflects fail2ban, port, and base path options', () => {
    ...
    expect(cmd).toContain('ghcr.io/mdaltoon10/Daltoon-UI:latest');
...
describe('buildDockerCompose', () => {
  it('produces valid-looking compose with the image and volumes', () => {
    ...
    expect(yaml).toContain('image: ghcr.io/mdaltoon10/Daltoon-UI:latest');
