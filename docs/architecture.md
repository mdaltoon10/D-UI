...
d-ui is a **web control panel for [Xray-core](https://github.com/XTLS/Xray-core)**. The Go
...
2. **The Runtime abstraction (multi-node).** A panel can manage remote "nodes" (other d-ui
...
- ORM: **GORM** with **SQLite** (default) or **PostgreSQL** (`DUI_DB_TYPE=postgres`).
  ...
  → POST {basePath}/panel/api/...
    → Gin engine (internal/web/web.go: initRouter)
      → middleware chain: SecurityHeaders → MaxBodyBytes (10 MiB; importDB exempt)
                          → [DomainValidator, if webDomain set] → gzip → sessions("d-ui")
...
│   │                           #   Every DUI_* env var is read here (config.go)
│   ├── database/
...
│   ├── tunnelmonitor/          # Optional tunnel health probe (DUI_TUNNEL_HEALTH_* env vars):
...
├── install.sh / update.sh / d-ui.sh                        # VPS install + management CLI
├── d-ui.service.*  / d-ui.rc                               # systemd units (debian/rhel/arch) + rc script
...
A "node" (`model.Node`) is another d-ui instance this panel controls. Every state-changing
...
(`DUI_TUNNEL_HEALTH_*`, read in `internal/config/`), deliberately independent of panel
...
| Xray auto-restart on **dead tunnel** | `internal/tunnelmonitor/` | `DUI_TUNNEL_HEALTH_*` in `internal/config/` |
...
- **SQLite vs Postgres.** Default is SQLite at `{DUI_DB_FOLDER}/d-ui.db`. Postgres via
  `DUI_DB_TYPE=postgres` + `DUI_DB_DSN`. Some SQL paths are dialect-aware (`database/dialect.go`);
