# Daltoon-UI — Architecture & Code Map
...
Daltoon-UI is a **web control panel for [Xray-core](https://github.com/XTLS/Xray-core)**. The Go
...
inbounds exist — the `mtg-multi` Telegram-proxy binary (`github.com/mdaltoon10/mtg-multi`, a
...
2. **The Runtime abstraction (multi-node).** A panel can manage remote "nodes" (other Daltoon-UI
  ...
  → POST {basePath}/panel/api/...
    → Gin engine (internal/web/web.go: initRouter)
      → middleware chain: SecurityHeaders → MaxBodyBytes (10 MiB; importDB exempt)
                          → [DomainValidator, if webDomain set] → gzip → sessions("Daltoon-UI")
...
A "node" (`model.Node`) is another Daltoon-UI instance this panel controls. Every state-changing
