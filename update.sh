...
# finished. Written to a fixed path outside DUI_MAIN_FOLDER so it survives
...
dui_update_run_id="${DUI_UPDATE_RUN_ID:-0}"
[[ "${dui_update_run_id}" =~ ^[0-9]+$ ]] || dui_update_run_id="0"
dui_update_status_file="${DUI_UPDATE_STATUS_FILE:-/etc/d-ui/update-status.json}"

_write_update_status() {
    local state="$1"
    local exit_code="$2"
    local status_dir
    status_dir="$(dirname "${dui_update_status_file}")"
    mkdir -p "${status_dir}" > /dev/null 2>&1
    local tmp_file="${dui_update_status_file}.tmp.$$"
    printf '{"runId":"%s","state":"%s","exitCode":%s,"finishedAt":%s}\n' \
        "${dui_update_run_id}" "${state}" "${exit_code}" "$(date +%s)" > "${tmp_file}" 2> /dev/null
    mv -f "${tmp_file}" "${dui_update_status_file}" > /dev/null 2>&1
}
...
prompt_and_setup_ssl() {
    ...
    case "$ssl_choice" in
        ...
        2)
            ...
            if [[ $release == "alpine" ]]; then
                rc-service d-ui restart > /dev/null 2>&1
            else
                systemctl restart d-ui > /dev/null 2>&1
            fi
...
config_after_update() {
    ...
    echo -e "${yellow}d-ui settings:${plain}"
...
# by invoking the freshly downloaded d-ui CLI. IP Limit is load-bearing on
...
# the update. DUI_ENABLE_FAIL2BAN is honored (load_xui_env exports it from the
...
update_d-ui() {
    ...
    if [ -f "${dui_folder}/d-ui" ]; then
        current_xui_version=$(${dui_folder}/d-ui -v)
        echo -e "${green}Current d-ui version: ${current_xui_version}${plain}"
    else
        _fail "ERROR: Current d-ui version: unknown"
    fi

    echo -e "${green}Downloading new d-ui version...${plain}"

    # DUI_UPDATE_TAG lets the panel target a specific release tag (e.g. the
    # rolling dev-latest pre-release). Empty keeps the default latest-stable flow.
    if [[ -n "${DUI_UPDATE_TAG}" ]]; then
    ...
    if [[ ! -s ${dui_folder}-linux-$(arch).tar.gz ]]; then
        rm ${dui_folder}-linux-$(arch).tar.gz -f > /dev/null 2>&1
        _fail "ERROR: Downloaded d-ui release archive is empty, please be sure that your server can access GitHub"
    fi

    if [[ -e ${dui_folder}/ ]]; then
        echo -e "${green}Stopping d-ui...${plain}"
        if [[ $release == "alpine" ]]; then
            if [ -f "/etc/init.d/d-ui" ]; then
                rc-service d-ui stop > /dev/null 2>&1
                rc-update del d-ui > /dev/null 2>&1
                echo -e "${green}Removing old service unit version...${plain}"
                rm -f /etc/init.d/d-ui > /dev/null 2>&1
            else
                rm d-ui-linux-$(arch).tar.gz -f > /dev/null 2>&1
                _fail "ERROR: d-ui service unit not installed."
            fi
        else
            if [ -f "${dui_service}/d-ui.service" ]; then
                systemctl stop d-ui > /dev/null 2>&1
                systemctl disable d-ui > /dev/null 2>&1
                echo -e "${green}Removing old systemd unit version...${plain}"
                rm ${dui_service}/d-ui.service -f > /dev/null 2>&1
                systemctl daemon-reload > /dev/null 2>&1
            else
                rm d-ui-linux-$(arch).tar.gz -f > /dev/null 2>&1
                _fail "ERROR: d-ui systemd unit not installed."
            fi
        fi
        ...
        echo -e "${green}Removing old d-ui version...${plain}"
        rm ${dui_folder} -f > /dev/null 2>&1
        rm ${dui_folder}/d-ui.service -f > /dev/null 2>&1
        rm ${dui_folder}/d-ui.service.debian -f > /dev/null 2>&1
        rm ${dui_folder}/d-ui.service.arch -f > /dev/null 2>&1
        rm ${dui_folder}/d-ui.service.rhel -f > /dev/null 2>&1
        rm ${dui_folder}/d-ui -f > /dev/null 2>&1
        rm ${dui_folder}/d-ui.sh -f > /dev/null 2>&1
        echo -e "${green}Removing old xray version...${plain}"
        rm ${dui_folder}/bin/xray-linux-amd64 -f > /dev/null 2>&1
        echo -e "${green}Removing old README and LICENSE file...${plain}"
        rm ${dui_folder}/bin/README.md -f > /dev/null 2>&1
        rm ${dui_folder}/bin/LICENSE -f > /dev/null 2>&1
    else
        rm d-ui-linux-$(arch).tar.gz -f > /dev/null 2>&1
        _fail "ERROR: d-ui not installed."
    fi

    echo -e "${green}Installing new d-ui version...${plain}"
    tar zxvf d-ui-linux-$(arch).tar.gz > /dev/null 2>&1
    if [[ $? -ne 0 ]]; then
        rm d-ui-linux-$(arch).tar.gz -f > /dev/null 2>&1
        _fail "ERROR: Failed to extract the d-ui release archive -- the previous installation has already been removed, so the panel will not start until this is fixed; try running the update again"
    fi
    rm d-ui-linux-$(arch).tar.gz -f > /dev/null 2>&1
    cd d-ui > /dev/null 2>&1
    if [[ $? -ne 0 || ! -s d-ui ]]; then
        _fail "ERROR: Extracted d-ui archive is missing the d-ui binary -- the previous installation has already been removed, so the panel will not start until this is fixed; try running the update again"
    fi
    chmod +x d-ui > /dev/null 2>&1
    ...
    chmod +x d-ui bin/xray-linux-$(arch) > /dev/null 2>&1

    echo -e "${green}Installing d-ui.sh script...${plain}"
    ...
    if [[ ! -s "${dui_script_temp}" ]]; then
        rm -f "${dui_script_temp}"
        _fail "ERROR: Failed to obtain d-ui.sh script"
    fi
    mv -f "${dui_script_temp}" /usr/bin/d-ui
    if [[ $? -ne 0 ]]; then
        rm -f "${dui_script_temp}"
        _fail "ERROR: Failed to install d-ui.sh script"
    fi

    chmod +x ${dui_folder}/d-ui.sh > /dev/null 2>&1
    chmod +x /usr/bin/d-ui > /dev/null 2>&1
    mkdir -p /var/log/d-ui > /dev/null 2>&1

    echo -e "${green}Changing owner...${plain}"
    chown -R root:root ${dui_folder} > /dev/null 2>&1

    if [ -f "${dui_folder}/bin/config.json" ]; then
        echo -e "${green}Changing on config file permissions...${plain}"
        chmod 640 ${dui_folder}/bin/config.json > /dev/null 2>&1
    fi

    if [[ $release == "alpine" ]]; then
        echo -e "${green}Installing startup unit d-ui.rc...${plain}"
        ...
        if [[ ! -s "${dui_rc_temp}" ]]; then
            rm -f "${dui_rc_temp}"
            _fail "ERROR: Failed to obtain startup unit d-ui.rc"
        fi
        mv -f "${dui_rc_temp}" /etc/init.d/d-ui
        if [[ $? -ne 0 ]]; then
            rm -f "${dui_rc_temp}"
            _fail "ERROR: Failed to install startup unit d-ui.rc"
        fi
        chmod +x /etc/init.d/d-ui > /dev/null 2>&1
        chown root:root /etc/init.d/d-ui > /dev/null 2>&1
        rc-update add d-ui > /dev/null 2>&1
        rc-service d-ui start > /dev/null 2>&1
    else
        ...
        # If service file still not installed, download from GitHub
        if [ "$service_installed" = false ]; then
        ...
        systemctl daemon-reload > /dev/null 2>&1
        systemctl enable d-ui > /dev/null 2>&1
        systemctl start d-ui > /dev/null 2>&1
    fi

    config_after_update

    # IP Limit relies on fail2ban; install + configure it now so the feature
    # works out of the box on update too (no-op when DUI_ENABLE_FAIL2BAN=false).
    # Never fatal.
    setup_fail2ban

    echo -e "${green}d-ui ${tag_version}${plain} updating finished, it is running now..."
    echo -e ""
    echo -e "┌───────────────────────────────────────────────────────┐
