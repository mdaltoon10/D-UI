...
for filepath in glob.glob("internal/web/translation/*.json"):
    ...
    if "pages" in data and "settings" in data["pages"] and "ipLimitPolicyOptions" in data["pages"]["settings"]:
        ...
        opts["block_newest"] = "Block Newest (Ban IP)"
        opts["block_newest_kick_only"] = "Block Newest (No Ban, Disconnect Only) [Default]"
        ...
        if "fa-IR" in filepath:
            ...
            opts["block_newest"] = "قطع اتصال جدیدترین (با مسدودسازی IP)"
            opts["block_newest_kick_only"] = "فقط قطع اتصال جدیدترین (بدون مسدودسازی IP) [پیش‌فرض]"
