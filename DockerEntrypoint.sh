...
# Start fail2ban with the dui-ipl jail
if [ "$DUI_ENABLE_FAIL2BAN" = "true" ]; then
    LOG_FOLDER="${DUI_LOG_FOLDER:-/var/log/d-ui}"
    mkdir -p "$LOG_FOLDER"
    touch "$LOG_FOLDER/duiipl.log" "$LOG_FOLDER/duiipl-banned.log"

    mkdir -p /etc/fail2ban/jail.d /etc/fail2ban/filter.d /etc/fail2ban/action.d

    cat > /etc/fail2ban/jail.d/dui-ipl.conf << EOF
[dui-ipl]
enabled=true
backend=auto
filter=dui-ipl
...
EOF
    ...
    PANEL_PORT=$(/app/d-ui setting -show true 2>/dev/null | grep -Eo 'port: .+' | awk '{print $2}')
    EXEMPT_PORTS="$SSH_PORTS"
    [ -n "$PANEL_PORT" ] && EXEMPT_PORTS="$EXEMPT_PORTS,$PANEL_PORT"

    cat > /etc/fail2ban/action.d/dui-ipl.conf << EOF
[INCLUDES]
...
actionban = <iptables> -I f2b-<name> 1 -s <ip> -p tcp -m multiport ! --dports <exemptports> -j <blocktype>
            ...
            echo "\$(date +"%%Y/%%m/%%d %%H:%%M:%%S")   BAN   [Email] = <F-USER> [IP] = <ip> banned for <bantime> seconds." >> $LOG_FOLDER/duiipl-banned.log
